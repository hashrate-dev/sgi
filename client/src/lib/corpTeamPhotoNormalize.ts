...
const TEAM_FRAME_BG = "#f3f4f6";
const TEAM_RING_COLOR = "#e5e7eb";

/** Geometría medida en JV-Team-1024x991.png */
const TEAM_CIRCLE_CY_RATIO = 0.4889;
const TEAM_CIRCLE_DIAM_RATIO = 688.5 / 1024;
...
function renderTeamPhotoCanvas(img: HTMLImageElement): HTMLCanvasElement {
  ...
  ctx.fillStyle = TEAM_FRAME_BG;
  ctx.fillRect(0, 0, w, h);

  const cx = w / 2;
  const cy = h * TEAM_CIRCLE_CY_RATIO;
  const lineWidth = Math.max(2, Math.round(w * 0.002));
  ...
  const strokeRadius = Math.max(1, radius - lineWidth);
  ctx.beginPath();
  ctx.arc(cx, cy, strokeRadius, 0, Math.PI * 2);
  ctx.strokeStyle = TEAM_RING_COLOR;
...
async function canvasToJpegFile(canvas: HTMLCanvasElement, baseName: string, quality: number): Promise<File> {
  ...
  return new File([blob], `${safeBase}-team.jpg`, { type: "image/jpeg" });
}
...
export async function normalizeCorpTeamPhotoFile(file: File): Promise<File> {
  ...
  let quality = hosted ? 0.8 : 0.88;
  let out = await canvasToJpegFile(canvas, file.name, quality);
  while (out.size > maxBytes && quality > 0.52) {
    quality -= 0.05;
