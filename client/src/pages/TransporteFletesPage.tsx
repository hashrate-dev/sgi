...
import {
  ...
  updateTransporteFleteTipo,
  deleteTransporteFleteTipo,
  type TransporteFleteTiposResponse,
} from "../lib/api";
import type { TransporteFleteTipo } from "../lib/types";
...
async function parseExcelTransporteFlete(file: File): Promise<{ nombre: string; precioUSD: number }[]> {
...
export function TransporteFletesPage() {
  const { user } = useAuth();
  const canEdit = user ? canEditClientes(user) : false;
  const canExportData = user ? canExport(user) : false;
  const [items, setItems] = useState<TransporteFleteTipo[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [deleteTarget, setDeleteTarget] = useState<TransporteFleteTipo | null>(null);
  const [showAddModal, setShowAddModal] = useState(false);
  const [editing, setEditing] = useState<TransporteFleteTipo | null>(null);
  const [excelLoading, setExcelLoading] = useState(false);
  const [formData, setFormData] = useState({ nombre: "", precioUSD: 0 });

  const load = () =>
    getTransporteFleteTipos()
      .then((r: TransporteFleteTiposResponse) => setItems(r.items || []))
      .catch(() => {
        setItems([]);
        showToast("Error al cargar ítems de transporte y fletes. Revisá la conexión con el servidor.", "error", "Transporte y fletes");
      });
  ...
  function handleSave() {
    if (!formData.nombre.trim()) {
      showToast("Completá el nombre del ítem.", "error", "Transporte y fletes");
      return;
    }
    ...
    if (editing) {
      const id = editing.id;
      updateTransporteFleteTipo(id, { nombre, precioUSD })
        .then(() => {
          showToast("Ítem actualizado correctamente.", "success", "Transporte y fletes");
          return getTransporteFleteTipos();
        })
        .then((r: TransporteFleteTiposResponse) => setItems(r.items || []))
        .catch((e) => showToast(e instanceof Error ? e.message : "Error al actualizar.", "error", "Transporte y fletes"))
        .finally(() => {
      ...
      createTransporteFleteTipo({ nombre, precioUSD })
        .then(() => {
          showToast("Ítem agregado correctamente.", "success", "Transporte y fletes");
          return getTransporteFleteTipos();
        })
        .then((r: TransporteFleteTiposResponse) => setItems(r.items || []))
        .catch((e) => showToast(e instanceof Error ? e.message : "Error al guardar.", "error", "Transporte y fletes"))
        .finally(() => {
          setShowAddModal(false);
          setFormData({ nombre: "", precioUSD: 0 });
        });
  ...
  function openEdit(s: TransporteFleteTipo) {
    setEditing(s);
  ...
  function handleDelete() {
    ...
    deleteTransporteFleteTipo(deleteTarget.id)
      .then(() => {
        showToast("Ítem eliminado correctamente.", "success", "Transporte y fletes");
        return getTransporteFleteTipos();
      })
      .then((r: TransporteFleteTiposResponse) => setItems(r.items || []))
      .catch((e) => showToast(e instanceof Error ? e.message : "Error al eliminar.", "error", "Transporte y fletes"))
      .finally(() => setDeleteTarget(null));
  }

  function exportExcel() {
    if (!canExportData) {
      showToast("No tenés permisos para exportar.", "error", "Transporte y fletes");
      return;
    }
    if (items.length === 0) {
      showToast("No hay ítems para exportar.", "warning", "Transporte y fletes");
      return;
    }
    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet("TransporteFletes");
    ...
    void wb.xlsx.writeBuffer().then((buffer) => {
      ...
      saveAs(new Blob([buffer]), `TransporteFletes-${fecha}.xlsx`);
      showToast("Archivo Excel exportado correctamente.", "success", "Transporte y fletes");
    });
  }
  ...
  function handleImportExcel(ev: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!/\.xlsx?$/i.test(file.name)) {
      showToast("Elegí un archivo Excel (.xlsx).", "error", "Transporte y fletes");
      return;
    }
    setExcelLoading(true);
    void (async () => {
      try {
        const parsed = await parseExcelTransporteFlete(file);
        if (parsed.length === 0) {
          showToast("No se encontraron filas válidas. Usá encabezados: Nombre, Precio USD.", "error", "Transporte y fletes");
          return;
        }
        for (const row of parsed) {
          await createTransporteFleteTipo({ nombre: row.nombre, precioUSD: row.precioUSD });
        }
        const r = await getTransporteFleteTipos();
        setItems(r.items || []);
        showToast(`Se importaron ${parsed.length} ítems.`, "success", "Transporte y fletes");
      } catch (err) {
        showToast(err instanceof Error ? err.message : "Error al importar Excel.", "error", "Transporte y fletes");
      } finally {
  ...
  return (
    <div className="fact-page">
      <div className="container">
        <PageHeader title="Gestión de Transporte y Fletes" />
              ...
              <div className="card clientes-filtros-card">
                ...
                <div className="row g-2 align-items-end">
                  ...
                  <div className="col-md-auto d-flex align-items-end gap-2 ms-auto">
                    ...
                    {canEdit && (
                      <>
                        <input type="file" accept=".xlsx,.xls" className="d-none" id="transporte-flete-import-excel" onChange={handleImportExcel} disabled={excelLoading} />
                        <label
                          htmlFor="transporte-flete-import-excel"
          ...
          <div className="clientes-listado-wrap">
            <div className="d-flex justify-content-between align-items-center mb-2">
              <h6 className="fw-bold m-0">
                🚚 Transporte y fletes ({loading ? "…" : filtered.length})
            ...
            {loading ? (
              ...
              <div className="fact-empty">
                ...
                <div className="fact-empty-text">
                  {searchTerm ? "No hay ítems con ese nombre." : 'No hay ítems de transporte/flete. Agregá uno con «Nuevo ítem».'}
                </div>
              </div>
            ) : (
            ...
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content fact-card" style={{ border: "none", borderRadius: "8px", overflow: "hidden" }}>
                <div className="fact-card-header d-flex align-items-center justify-content-between">
                  <span>{editing ? "Editar ítem" : "Nuevo ítem de transporte / flete"}</span>
                  <button
                    type="button"
                    className="btn-close"
                    onClick={() => {
                ...
                <div className="fact-card-body">
                  ...
                  <div className="fact-field">
                    ...
                    <input
                      ...
                      placeholder="Ej. Flete CDE–Asunción"
                    />
                  </div>
                  <div className="fact-field">
                    <label className="fact-label" htmlFor="transporte-flete-precio-usd">
                      Precio USD *
                    </label>
                    <input
                      id="transporte-flete-precio-usd"
                      type="number"
                      min={0}
                      max={99999}
                      step={1}
            ...
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content historial-delete-modal">
                ...
                <div className="modal-body historial-delete-modal-body">
                  <p className="historial-delete-question">¿Seguro que querés eliminar este ítem?</p>
