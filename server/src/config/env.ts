...
(() => {
  ...
  if (apiKey && from && !badResendKey) {
    ...
    // eslint-disable-next-line no-console
    console.warn(
      "[email] Vercel: RESEND_API_KEY no está definida — los avisos de «Generar orden» no se enviarán. Configurala en Vercel → Settings → Environment Variables (y RESEND_FROM_EMAIL con un remitente @mail.hashrate.space o @hashrate.space verificado en Resend)."
    );
  } else if (devConsole) {
    // eslint-disable-next-line no-console
    console.log(
