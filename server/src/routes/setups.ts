...
setupsRouter.get("/setups", requireAuth, async (_req, res: Response) => {
  try {
    ...
    const rows = (await db.prepare("SELECT id, codigo, nombre, precio_usd AS precioUSD FROM setups ORDER BY codigo ASC, nombre ASC").all()) as SetupRow[];
...
setupsRouter.delete("/setups", requireAuth, requireRole("admin_a", "admin_b"), async (_req, res: Response) => {
  try {
    await db.prepare("DELETE FROM setups").run();
    res.json({ ok: true });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    res.status(500).json({ error: { message: msg } });
